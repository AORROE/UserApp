...
export default class OrderUndonePage extends Component{
    constructor(props){
        super(props);
        this.state={
            data:[],
    ...
    getData(){
        Storage.get('token',(value)=>{
            Storage.get('shopInfo',(valueid)=>{
                NT.get("http://101.132.71.227/api/app/getUndoOrder/"+valueid[0].id,(res)=>{
        ...
        return(
            <TouchableOpacity
                ...
                <View style={{flexDirection:'row',padding:10,justifyContent:'flex-end',width:screenWidth}}>
                    ...
                    <TouchableOpacity
                        style={{width:80,height:30,borderColor:'#b7b7b7',borderWidth:1,
                            alignItems:'center',justifyContent:'center',borderRadius:2}}>
                        <Text style={{color:'#b7b7b7'}}>无效</Text>
                    </TouchableOpacity>
                </View>
            </TouchableOpacity>
        );
